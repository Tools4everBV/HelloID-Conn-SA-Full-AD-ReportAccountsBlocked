...
'@ 
Invoke-HelloIDGlobalVariable -Name "ADusersReportOU" -Value $tmpValue -Secret "False" 
$tmpValue = @'
C:\HIDreports\
'@ 
...
<# Begin: DataSource "AD-user-generate-table-report-disabled-locked" #>
$tmpPsScript = @'
try {
...
'@ 
$tmpModel = @'
[{"key":"Enabled","type":0},{"key":"SamAccountName","type":0},{"key":"Title","type":0},{"key":"LockedOut","type":0},{"key":"Department","type":0},{"key":"Displayname","type":0},{"key":"UserPrincipalName","type":0},{"key":"CanonicalName","type":0}]
'@ 
$tmpInput = @'

'@ 
$dataSourceGuid_0 = [PSCustomObject]@{} 
Invoke-HelloIDDatasource -DatasourceName "AD-user-generate-table-report-disabled-locked" -DatasourceType "4" -DatasourceInput $tmpInput -DatasourcePsScript $tmpPsScript -DatasourceModel $tmpModel -returnObject ([Ref]$dataSourceGuid_0) 
<# End: DataSource "AD-user-generate-table-report-disabled-locked" #>
<# End: HelloID Data sources #>

<# Begin: Dynamic Form "AD - Report - Blocked accounts (disabled or locked)" #>
$tmpSchema = @"
[{"templateOptions":{},"type":"markdown","summaryVisibility":"Show","body":"The following report will show local AD accounts that are blocked or locked. Please wait while the data is loading...","requiresTemplateOptions":false},{"key":"grid","templateOptions":{"label":"Results","grid":{"columns":[{"headerName":"CanonicalName","field":"CanonicalName"},{"headerName":"Displayname","field":"Displayname"},{"headerName":"UserPrincipalName","field":"UserPrincipalName"},{"headerName":"Department","field":"Department"},{"headerName":"Title","field":"Title"},{"headerName":"Enabled","field":"Enabled"},{"headerName":"Locked","field":"LockedOut"}],"height":500,"rowSelection":"single"},"dataSourceConfig":{"dataSourceGuid":"$dataSourceGuid_0","input":{"propertyInputs":[]}},"useFilter":true,"useDefault":false},"type":"grid","summaryVisibility":"Hide element","requiresTemplateOptions":true},{"key":"exportReport","templateOptions":{"label":"Export report (local export on HelloID Agent server)","useSwitch":true,"checkboxLabel":"Yes","mustBeTrue":true},"type":"boolean","summaryVisibility":"Show","requiresTemplateOptions":true}]
"@ 

$dynamicFormGuid = [PSCustomObject]@{} 
Invoke-HelloIDDynamicForm -FormName "AD - Report - Blocked accounts (disabled or locked)" -FormSchema $tmpSchema  -returnObject ([Ref]$dynamicFormGuid) 
<# END: Dynamic Form #>
...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD - Report - Blocked accounts (disabled or locked)" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-info-circle" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
...
'@ 
	...
	Invoke-HelloIDAutomationTask -TaskName "AD-export-report-accounts-disabled-locked" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD - Report - Blocked accounts (disabled or locked)' already exists. Nothing to do with the Delegated Form task..." 
}
